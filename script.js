...
const caseHud = document.querySelector(".case-hud");
...
function activateCaseStudyLink(link, { keyboard = false } = {}) {
  const group = link.closest(".archive-group");
  const trigger = group?.querySelector("[data-archive-trigger]");
  ...
  // Activate the record before making its touch menu inert. Mobile Safari can
  // otherwise drop the delayed hash navigation when the submenu closes.
  setCaseStudy(link);
...
let hudTouch = null;
const suppressedHudClicks = new WeakMap();

function openArchiveFromTouch(trigger) {
  const group = trigger.closest(".archive-group");
  ...
  delete group.dataset.timedMenu;
  group.classList.remove("is-dismissed");
  setArchiveMenu(trigger, open, { announce: false });
...
caseHud.addEventListener(
  "touchstart",
  (event) => {
    const control = event.target.closest(
      ".profile-nav-trigger, [data-archive-trigger], .archive-submenu a[data-case-study]",
    );

    if (!control || !caseHud.contains(control) || event.touches.length !== 1) {
      hudTouch = null;
      return;
    }

    const touch = event.touches[0];
    hudTouch = { control, x: touch.clientX, y: touch.clientY };

    const group = control.closest(".archive-group");
    if (group) {
      cancelArchiveClose(group);
    }
...
caseHud.addEventListener(
  "touchend",
  (event) => {
    if (!hudTouch || event.changedTouches.length !== 1) {
      hudTouch = null;
      return;
    }

    const { control, x, y } = hudTouch;
    const touch = event.changedTouches[0];
    hudTouch = null;
    event.stopPropagation();

    if (Math.hypot(touch.clientX - x, touch.clientY - y) > 32) {
      return;
    }

    event.preventDefault();
    suppressedHudClicks.set(control, Date.now() + 700);

    if (control.matches(".archive-submenu a[data-case-study]")) {
      activateCaseStudyLink(control);
    } else if (control.matches("[data-archive-trigger]")) {
      openArchiveFromTouch(control);
    } else {
      openProfileFromTouch();
    }
  },
...
caseHud.addEventListener(
  ...
  (event) => {
    ...
    if (control && Date.now() < (suppressedHudClicks.get(control) || 0)) {
      event.preventDefault();
...
caseStudyLinks.forEach((link) => {
  link.addEventListener("click", (event) => {
    event.preventDefault();
    activateCaseStudyLink(link, { keyboard: event.detail === 0 });
  });

  link.addEventListener("keydown", (event) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      activateCaseStudyLink(link, { keyboard: true });
